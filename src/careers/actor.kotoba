(ns careers.actor
  "CareersActor — the ISCO-08 2423 community careers actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one careers operation request
  ...
  The unconditional invariant: the CareersAdvisor can never
  directly commit a record the CareersGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [careers.advisor :as advisor]
            [careers.governor :as governor]
            [careers.store :as store]))

(defn build-graph
  "Build a compiled CareersActor graph. `store` implements
  `careers.store/Store`. `advisor` implements
  `careers.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :candidate-id (:candidate-id proposal)
                                    :payload proposal}]
